...
6. Shared project or company facts that belong on a named standalone graph: `add_memory_to_graph` with that `graph_id` when that write tool is present. Do not put personal preferences on a company graph.
